...
public:
    ...
    ~List()
    {
        ...
        if(head) delete head;
        if(tail) delete tail;
        cout<<"the process is constructing"<<endl; //为了测试析构函数
    }
...
/*    List &operator=(const List &rhs)
    ...
    //添加resize功能，第一个参数代表改变后的链表thesize大小，第二个参数代表默认的填充数据，否则对列表截尾
    void resize(int n, const Object &a){
        while(n>theSize){
            push_back(a);
        }
        while(n<theSize){
    ...
    //添加查找元素功能, 参数代表元素的值，如果找到返回该元素所在位置,把currentpos设成该位置,如果没找到返回-1
    int find(const Object &val){
        int i=0;
        auto itr=begin();
        while (itr!=end()){
            if (*itr==val){
